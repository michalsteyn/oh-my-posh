...
function Write-Theme {
    ...
    # Add GitVersion, use git describe to track if git changed. Improves performance
		$describe = git describe --long --all
		if($describe -ne $global:gitdescribe) {
			$global:gitdescribe = $describe
			$gitVersion = gitversion /showvariable SemVer
		...
		$prompt += Write-Prompt -Object " $gitVersion " -ForegroundColor $sl.Colors.PromptForegroundColor -BackgroundColor $sl.Colors.PromptBackgroundColor
		$lastColor = $sl.Colors.PromptBackgroundColor
    }
